...
public class Controller {
	...
	private boolean isWinning(CellState player, int col, int row) {
		...
		if (checkHorizontal(player, col, row) || checkVertical(player, col, row) || checkDiagonalAscending(player, col, row)
	...
	private boolean checkVertical(CellState player, int col, int row) {
		...
		if (row + (model.getWinLength()) < model.numOfRows()) {
			...
			if (model.getCell(col, row + 1) == player && model.getCell(col, row + 2) == player && model.getCell(col, row + 3) == player
